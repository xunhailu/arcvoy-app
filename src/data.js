export const JOBS = [
  {
    id: 1,
    title: 'AI Search Specialist',
    dept: 'AI',
    type: 'Remote',
    salary: '$25 / hr',
    desc: 'Help improve how AI search systems understand and surface results. You will evaluate queries, refine prompts, and contribute to training retrieval models used in real, live products.',
    reqs: ['Search evaluation', 'Prompt engineering', 'Attention to detail', 'AI tools familiarity'],
    bonus: ['Experience with LLMs', 'Data annotation'],
    locations: ['Worldwide'],
  },
  {
    id: 2,
    title: 'Social Media Evaluator',
    dept: 'AI',
    type: 'Remote',
    salary: '$25 / hr',
    desc: 'Review and rate social media search results across images, video, and advertising content. Your input directly shapes the accuracy and relevance of AI content systems used by millions.',
    reqs: ['Content evaluation', 'Analytical skills', 'Internet savvy', 'Consistency'],
    bonus: ['Social media knowledge', 'AI dataset work'],
    locations: ['Worldwide'],
  },
  {
    id: 3,
    title: 'Project Unicorn',
    dept: 'AI',
    type: 'Remote',
    salary: '$20 / hr',
    desc: 'Join a confidential AI initiative working on experimental workflows and next-generation automation. You will contribute to systems still taking shape, using advanced tooling at the forefront of the field.',
    reqs: ['Adaptability', 'Problem solving', 'Curiosity', 'Tech awareness'],
    bonus: ['Startup experience', 'AI experimentation'],
    locations: ['Worldwide'],
  },
  {
    id: 4,
    title: 'Customer Care Specialist',
    dept: 'Support',
    type: 'Hybrid',
    salary: '$18 / hr',
    desc: 'Represent Arcvoy as the first point of contact for clients and candidates. Handle inquiries with speed and care, resolve issues thoroughly, and uphold our standard for every interaction.',
    reqs: ['Communication skills', 'Problem solving', 'Empathy', 'CRM experience'],
    bonus: ['Recruitment background', 'Multilingual'],
    locations: ['United States', 'United Kingdom', 'Canada'],
  },
  {
    id: 5,
    title: 'Data Analyst',
    dept: 'Analytics',
    type: 'On-site',
    salary: '$30 / hr',
    desc: 'Transform hiring and operations data into clear, actionable insights. Build dashboards, surface trends, and help leadership make decisions grounded in real numbers across the business.',
    reqs: ['SQL', 'Data visualisation', 'Analytical thinking', 'Excel / Sheets'],
    bonus: ['Python or R', 'BI tools (Tableau, Looker)'],
    locations: ['United States'],
  },
]

export const PILLARS = [
  { num: '01', title: 'Remote first', text: 'Every role is built for focused, independent work. No office politics, no commutes.' },
  { num: '02', title: 'High ownership', text: 'We hire people who get things done without being managed every step of the way.' },
  { num: '03', title: 'Straight answers', text: 'You will know where your application stands within 48 hours. That is a promise, not a policy.' },
]

export const FEATURES = [
  {
    title: 'Fast turnaround',
    text: 'We do not sit on applications. Most candidates hear back within 48 hours — not because we rush it, but because we know what we are looking for.',
    icon: 'lightning',
  },
  {
    title: 'Genuinely global',
    text: 'Contributors across 40+ countries, working in every time zone. Wherever your team runs, we have people already in it.',
    icon: 'globe',
  },
  {
    title: 'Properly vetted',
    text: 'Every person goes through evaluation before they reach you. No guessing on day one — you know exactly what you are getting.',
    icon: 'lock',
  },
]

export const SPONSORS = [
  { name: 'Figma',     color: '#1E1E1E' },
  { name: 'StubHub',   color: '#7B2FF2' },
  { name: 'Zapier',    color: '#FF4A00' },
  { name: 'Intercom',  color: '#1F8DED' },
  { name: 'Stripe',    color: '#635BFF' },
  { name: 'Spotify',   color: '#1DB954' },
  { name: 'Brex',      color: '#1A1A1A' },
  { name: 'Shopify',   color: '#96BF48' },
]

export const HOW_IT_WORKS = [
  {
    step: '01',
    title: 'Apply in minutes',
    text: 'Pick a role, submit your details. No long forms, no hoops. Just tell us who you are and what you have done.',
    icon: 'apply',
  },
  {
    step: '02',
    title: 'Hear back fast',
    text: 'We go through every application within 48 hours. If there is a fit, we reach out directly with clear next steps. No ghosting, no silence.',
    icon: 'match',
  },
  {
    step: '03',
    title: 'Get to work',
    text: 'Onboard quickly, work on your schedule, and get paid every week. You join a global team doing real work on live AI systems.',
    icon: 'start',
  },
]

export const TESTIMONIALS = [
  {
    quote: "Arcvoy made it easy to find consistent AI evaluation work. The onboarding was straightforward and the team actually responds, which is not something you take for granted.",
    name: 'Priya M.',
    role: 'AI Search Specialist',
    location: 'San Francisco, US',
    initials: 'PM',
  },
  {
    quote: "I have worked with three different platforms. Arcvoy is the only one where I felt treated like a professional. Great pay, clear tasks, and someone always available if you need help.",
    name: 'Lucas B.',
    role: 'Social Media Evaluator',
    location: 'Paris, France',
    initials: 'LB',
  },
  {
    quote: "Within a week of applying I was already working on a live AI project. The 48-hour response promise is real. They said it and they did it.",
    name: 'Amara O.',
    role: 'Data Analyst',
    location: 'Lagos, Nigeria',
    initials: 'AO',
  },
]

export const ABOUT_TEXT = [
  'AI gets better because people make it better. That is the honest version of what we do.',
  'Behind every search result and every chatbot response is a person who tested it, trained it, and flagged when it got something wrong. We find those people, hire them, and pay them well for it.',
  'Our contributors work on live AI projects — search quality, output evaluation, edge case testing. They are careful, consistent, and take the work seriously. That is what we hire for.',
  'We help companies who need that kind of talent quickly, without the overhead of sourcing it themselves.',
  'That is the whole model. Straightforward, on purpose.',
]
